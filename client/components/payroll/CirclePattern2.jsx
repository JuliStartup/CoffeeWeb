...
const CirclePattern2 = () => {
  ...
  useEffect(() => {
    ...
    const sortedCircles = Array.from(allPopCircles).sort((a, b) => {
      const groupA = a.closest(".circle-group");
      const groupB = b.closest(".circle-group");
      const rowA = parseInt(groupA.dataset.row);
      const colA = parseInt(groupA.dataset.col);
      const rowB = parseInt(groupB.dataset.row);
      const colB = parseInt(groupB.dataset.col);

      const distanceA = getDistanceFromCenter(rowA, colA);
      const distanceB = getDistanceFromCenter(rowB, colB);

      return distanceB - distanceA; // Sort descending by distance
    });
    ...
    const startClusterAnimations = () => {
      if (activeCircles.size < MAX_ACTIVE_CIRCLES) {
        const availableCircles = sortedCircles.filter(
          (circle) => !activeCircles.has(circle)
        );

        if (availableCircles.length > 0) {
          const randomIndex = Math.floor(
            Math.random() * availableCircles.length
          );
